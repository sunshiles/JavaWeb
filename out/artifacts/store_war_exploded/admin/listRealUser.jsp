...
<form method="post" action="../RealUserServlet?operate=queryByPage" id="listform">
    ...
    <table class="table table-hover text-center">
        ...
        <volist name="list" id="vo">
                <%
                    ...
                    ArrayList<RealUser> realUsers = (ArrayList<RealUser>) request.getAttribute("realUsers");
                    if (realUsers != null) {
                         for (RealUser user : realUsers) {
%>
            <tr>
                ...
                <td>
                    <div class="button-group">
                        <a class="button border-main"
                           href="../RealUserServlet?operate=queryById&id=<%=user.getId()%>"><span
                ...
                <td colspan="8">
                    <div class="pagelist">
                        <%
                            if (currentPage > 1) {
                        %>
                        <a href="../RealUserServlet?operate=queryByPage&currentPage=<%=currentPage-1%>&username=<%=username%>">上一页</a>
                        <%
                            }
                        %>
                        <%
                        ...
                        %>
                        <a href="../RealUserServlet?operate=queryByPage&currentPage=<%=i%>&username=<%=username%>"><%=i%>
                        </a>
                        <%
                            }
                            if (currentPage < pageCount) {
                        %>
                        <a href="../RealUserServlet?operate=queryByPage&currentPage=<%=currentPage+1%>&username=<%=username%>">下一页</a>
                        <%
                            }
                        %>
                        <a href="../RealUserServlet?operate=queryByPage&currentPage=<%=pageCount%>&username=<%=username%>">尾页</a>
                    </div>
                </td>
            </tr>
    </table>
...
<script type="text/javascript">
    ...
    function del(idValues) {
        if (confirm("您确定要删除吗?")) {
            var newUrl = "../RealUserServlet?operate=del&id=" + idValues;
            $("#listform").attr('action', newUrl);    //通过jquery为action属性赋值
            $("#listform").submit();    //提交ID为myform的表单
        }
    }
