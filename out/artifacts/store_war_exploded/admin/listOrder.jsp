...
<form method="post" action="../OrderServlet?operate=queryByPage" id="listform">
...
%>
            <tr>
                ...
                <td><font><%=user.getUserId()%>

                </font></td>
                <td><%=user.getStatus()%>
                </td>
                <td><%=user.getOrderTime()%>
                </td>
                <td>
                    <div class="button-group">
                        <a class="button border-main"
                           href="../OrderServlet?operate=queryById&id=<%=user.getId()%>"><span
                ...
                <td colspan="8">
                    <div class="pagelist">
                        <%
                            if (currentPage > 1) {
                        %>
                        <a href="../OrderServlet?operate=queryByPage&currentPage=<%=currentPage-1%>&username=<%=username%>">上一页</a>
                        <%
                            }
                        %>
                        <%
                        ...
                        %>
                        <a href="../OrderServlet?operate=queryByPage&currentPage=<%=i%>&username=<%=username%>"><%=i%>
                        </a>
                        <%
                            }
                            if (currentPage < pageCount) {
                        %>
                        <a href="../OrderServlet?operate=queryByPage&currentPage=<%=currentPage+1%>&username=<%=username%>">下一页</a>
                        <%
                            }
                        %>
                        <a href="../OrderServlet?operate=queryByPage&currentPage=<%=pageCount%>&username=<%=username%>">尾页</a>
                    </div>
                </td>
            </tr>
    </table>
...
<script type="text/javascript">
    ...
    function del(idValues) {
        if (confirm("您确定要删除吗?")) {
            var newUrl = "../OrderServlet?operate=del&id=" + idValues;
            $("#listform").attr('action', newUrl);    //通过jquery为action属性赋值
            $("#listform").submit();    //提交ID为myform的表单
        }
    }
