...
@WebServlet("/RealUserServlet")
public class RealUserServlet extends HttpServlet {
...
//        String keywords = request.getParameter("keywords");
        ...
        if ("login".equals(operate)) {
            ...
            RealUserDao userDao = DaoFactory.getRealUserDao();
            int flag = userDao.verify(realName, password);
            String loginInfo = "";
            if (flag == 1) {
                loginInfo = "用户名不存在";
            } else if (flag == 2) {
                ...
                RealUser realUser = userDao.queryByName(realName);
                session.setAttribute("realUser", realUser);
                response.addCookie(new Cookie("username", realName));
                response.addCookie(new Cookie("password", password));
            }
            session.setAttribute("loginInfo",loginInfo);
            response.sendRedirect("buy/checkout.jsp");
        }
...
//            realUser.setId(idInt);
        ...
        if ("del".equals(operate)) {
            ...
            String id = request.getParameter("id");
            RealUserDao realUserDao = DaoFactory.getRealUserDao();
            boolean isTrue = realUserDao.delUser(id);
            if (isTrue) {
                ArrayList<RealUser> realUsers = realUserDao.queryByPage(currentPage, username);
                int pageCount = realUserDao.queryPageCount(username);
                request.setAttribute("realUsers", realUsers);
                request.setAttribute("currentPage", currentPage);
                request.setAttribute("pageCount", pageCount);
                request.getRequestDispatcher("admin/listRealUser.jsp").forward(request, response);
            } else {
