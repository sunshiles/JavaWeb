...
public class RealUserDaoImpl implements RealUserDao {
...
//            打印查询出来的东西
    ...
    public RealUser queryByName(String name) {
        ...
        try {
            ...
            if (resultSet.next()) {
                realUser.setId(resultSet.getInt("id"));
                realUser.setUserName(resultSet.getString("userName"));
                realUser.setPassword(resultSet.getString("password"));
                realUser.setRealName(resultSet.getString("realName"));
                realUser.setSex(resultSet.getString("sex"));
                realUser.setFavorate(resultSet.getString("favorate"));
                realUser.setAddress(resultSet.getString("address"));
                realUser.setEmail(resultSet.getString("email"));
                realUser.setStatus(resultSet.getInt("status"));
                realUser.setScore(resultSet.getInt("score"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public int queryPageCount(String name) {
        ...
        con = DBConn.getConnection();
        //执行查询语句
        String sql = "select count(*) from user_info where userName like ?;";
        try {
            preparedStatement = con.prepareStatement(sql);
            preparedStatement.setString(1, "%" + name + "%");
        } catch (SQLException e) {
...
//            preparedStatement.setString(2, password);
    ...
    @Override
    public ArrayList<RealUser> queryByPage(int currentPage, String name) {
        con = DBConn.getConnection();
        ArrayList<RealUser> realUsers = new ArrayList<>();
        //执行查询语句
        String sql = "select * from user_info where userName like ? limit ?,?;";
        try {
            preparedStatement = con.prepareStatement(sql);
        } catch (SQLException e) {
            e.printStackTrace();
